...
class ImageProjection(Node):
    ...
    def road_dots(self, image, what_to_do):
        ...
        if what_to_do == 'slow':
            heights = [h - 10, h - 60, h - 110]
            no_width = 2
            widths = [70, -1]

        if what_to_do == 'slow_out':
            heights = [h - 10, h - 60, h - 110]
            no_width = 1
            widths = [70, 700]
        ...
        if what_to_do == 'slow' or what_to_do == 'slow_out':
            self.center_line = []
            self.center_line.append([w // 7 * 2 + 105, 0])
            self.center_line.append([w // 7 * 2 + 105, h])
        ...
        for z in range(len(heights)):
            ...
            for threshold in thresholds:
                ...
                if start is not None and end is not None:
                    ...
                    if no_width == 2 and threshold == threshold_yellow:
                        width = widths[0]
                    ...
                    if fl == 0:
                        if z > 0:
                            if threshold == threshold_yellow:
                                ...
                                if what_to_do == 'slow' or what_to_do == 'slow_out':
                                    dot_mas.append([dot_mas[2*z - 1][0], heights[1]])
                                else:
                                    dot_mas.append([dot_mas[2*z - 1][0], dot_mas[2*z - 1][1]])
                        else:
                            if threshold == threshold_yellow:
        ...
        if (what_to_do == 'slow' or what_to_do == 'slow_out') and dot_mas[1] == dot_mas[3] == dot_mas[5]:
            angle = (angle / angle_coef + angle_center_line / center_coef) / 2 + np.radians(7)
        else:
            angle = (angle / angle_coef + angle_center_line / center_coef) / 2
        
        if what_to_do == 'slow_out' and not(dot_mas[1] == dot_mas[3] == dot_mas[5]):
            self.msg.data = 1
        
        if what_to_do == 'exit' and dot_mas[0] != [0, h] and dot_mas[1] != [w, h]:
            self.msg.data = 7
    ...
    def main_misl(self, msg_img):
        ...
        if self.msg.data == 4:
            self.road_dots(cv_image_homography, 'slow')
            self.msg.data = 1
            self.publisher_ride.publish(self.msg)
            self.msg.data = 4

        if self.msg.data == 5:
            self.road_dots(cv_image_homography, 'slow_out')
            if self.msg.data == 1:
                self.msg.data = 1
                self.publisher_ride.publish(self.msg)
            else:
        ...
        if self.msg.data == 6:
            self.road_dots(cv_image_homography, 'exit')
            self.msg.data = 1
            self.publisher_ride.publish(self.msg)
            self.msg.data = 6
        ...
        #     self.publisher_ride.publish(msg)

        #     self.msg.data = 1

        cv2.imshow("homography", cv_image_homography)
        cv2.imshow("original", cv_image_original)
