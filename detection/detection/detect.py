...
class LidarGraphMaker(Node):
    def __init__(self):
        ...
        self.robot_finish = self.create_publisher(
            String,
            '/robot_finish',
            10
        )
    ...
    def work_move(self, data, current_pos, current_orient):
        ...
        if self.state == 0:# прямо до препятствия
            ...
            if current_orient['z'] > -170 + inertial_angle_bias:
                vel_msg.angular.z = angular_speed
            else:
                self.moving_start_pos = None
                self.state += 1
    ...
    def parking_move(self, data, current_pos, current_orient):
        ...
        car_distance = 1.6
        ang_bias = 90
        print(self.state, left_data.mean(), right_data.mean())
        print(left_data.mean(), right_data.mean())
        if self.state == 0:
            if not (-5 + ang_bias < current_orient['z'] < 5 + ang_bias):
            ...
            length = self.start_moving(current_pos=current_pos)
            if left_data.mean() > car_distance and right_data.mean() > car_distance:
                vel_msg.linear.x = linear_speed 
            else:
                self.moving_start_pos = None
                self.state += 1 
            ...
            if self.parking_direction is None:
                if left_data.mean() <= car_distance:
                    print('left')
                    self.parking_direction = True
                elif right_data.mean() <= car_distance:
                    print('right')
                    self.parking_direction = False
            length = self.start_moving(current_pos=current_pos)
            if self.parking_direction:
                if not (135 < current_orient['z'] < 165):
                    vel_msg.angular.z = -angular_speed
                else:
                    self.moving_start_pos = None
                    self.state += 1 
            ...
            if self.parking_direction:
                ...
                if not (-145 < current_orient['z'] < -115):
                    vel_msg.angular.z = angular_speed
                else:
                    self.moving_start_pos = None
                    self.state += 1 
    ...
    def pedestrian_move(self, data, current_pos, current_orient):
        ...
        window_size = 10
        forward_data = data[180-window_size:180+window_size]
        forward_data1 = data[180+20-window_size:180+20+window_size]
        forward_data2 = data[180-20-window_size:180-20+window_size]
        linear_speed = 0.2
        angular_speed = 0.3
        if self.state == 0:
            length = self.start_moving(current_pos=current_pos)
            if forward_data.min() > 0.2 and forward_data1.mean() > 0.2 and forward_data2.mean() > 0.2 and length < 1.0:
                vel_msg.linear.x = linear_speed
            else:
                self.moving_start_pos = None
    ...
    def pass_tunnel(self, laser_data: LaserScan):
        ...
        wide_view   = data[170 - 20:190 + 20].mean()
        ...
        lefter  = data[260:278].mean()
        righter = data[90:110].mean()

        if self.flag == 1 and lefter > 1.8 and righter > 1.9:
            msg = String()
            msg.data = 'SkufLords'
            self.robot_finish.publish(msg)
        
        print(f'right_view: {righter}')
        print(f'left_view:  {lefter}')
    ...
    def image_callback(self, msg: Image):
        if (self.signs['work'] and not self.passed_work) or \
           (self.signs['pedestrian'] and not self.passed_pedestrian) or \
           (self.signs['parking'] and not self.passed_parking) or \
           (self.signs['tunnel'] and not self.passed_tunnel):
            return
        if (self.depth_image is not None and 7 < self.avg_depth.mean() < 20 or \
            (self.passed_pedestrian and not self.signs['tunnel']) or \
            (self.signs['t'] and not self.signs['direction'])) or \
            (not self.passed_parking and self.signs['parking']):
            image = self.bridge.imgmsg_to_cv2(msg, 'bgr8')
            yolo_result = self.model(image, verbose=False)[0]
            boxes = []
            mask = self.avg_depth
            for box in yolo_result.boxes:
                ...
                if label == 'hummer_front' and width * height < 10000:
                    continue
                if (width * height > 6000) and \
            ...
            if len(boxes) > 0:
                ...
                if boxes[-1]['label'] == "T_crossroad" and not self.signs['t']:
                    ...
                    ans.data = -1
                    self.check_publisher.publish(ans)
                elif boxes[-1]['label'] == "parking_sign" and not self.signs['parking'] and self.passed_work:
                    print("parking")
                    time.sleep(20)
                    ans = Int32()
                    ans.data = -1
                    ...
                    ans.data = -1
                    self.check_publisher.publish(ans)
                elif boxes[-1]['label'] == 'tunnel_sign':
                    print("tunnel")
                    ans = Int32()
